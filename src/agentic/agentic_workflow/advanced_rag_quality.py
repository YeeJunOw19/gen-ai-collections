...
def code_generation(state):
    ...
    answer = competitors.competitor_generation(question, competitor_name, rag)
...
def basic_agentic_workflow():
    ...
    workflow.add_edge("code_extraction", "code_quality")
    workflow.add_edge("code_quality", END)

    return workflow


def advanced_agents_workflow():
    # Initialize the workflow
    workflow = StateGraph(GraphState)
