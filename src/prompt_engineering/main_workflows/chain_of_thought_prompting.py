...
@asset(deps=[role_based_prompting.role_prompting])
async def chain_prompting() -> dict:
    ...
    system_examples = EXTERNAL_SCRIPTS.joinpath(CONFIG["Few_Shots_Template"]).resolve()
    with open(system_prompt, mode="r") as f:
        content = f.read().split("\n")
        combined = " ".join(content)

    with open(system_examples, mode="r") as f:
        examples = f.read()
...
@asset(ins={"openai_output": AssetIn(key="chain_prompting")})
def chain_scoring(openai_output):
    """
